...
import com.unimarket.presentation.*
...
import com.unimarket.presentation.buyer.*
import com.unimarket.presentation.common.ProfileScreen
import com.unimarket.presentation.seller.*
...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent { UniMarketApp(tokenManager, api, db, startDest) }
    }
}
...
fun UniMarketApp(
    ...
    val authVM   = viewModel<AuthViewModel>  (factory = AuthViewModelFactory(authRepo, tokenManager))
    val buyerVM  = viewModel<BuyerViewModel> (factory = BuyerViewModelFactory(listingRepo, cartRepo, orderRepo))
    val sellerVM = viewModel<SellerViewModel>(factory = SellerViewModelFactory(listingRepo))
    val adminVM  = viewModel<AdminViewModel> (factory = AdminViewModelFactory(adminRepo))
    ...
    NavHost(navController = navController, startDestination = startDest) {
        ...
        composable(Routes.LOGIN) {
            LoginScreen(
                viewModel  = authVM,
                onSuccess  = { role -> routeByRole(role) },
        ...
        composable(Routes.BUYER_BROWSE) {
            BrowseScreen(
                viewModel     = buyerVM,
                onViewListing = { listing -> selectedListing = listing; navController.navigate(Routes.BUYER_DETAIL) },
        ...
        composable(Routes.BUYER_CONFIRMATION) {
            OrderConfirmationScreen(
                viewModel = buyerVM,
                onDone    = { navController.navigate(Routes.BUYER_BROWSE) { popUpTo(Routes.BUYER_BROWSE) { inclusive = true } } }
            )
        }
        ...
        composable(Routes.SELLER_DASHBOARD) {
            SellerDashboardScreen(
                ...
                onEdit       = { listing -> editingListing = listing; navController.navigate(Routes.SELLER_EDIT) },
                onLogout     = { logout() }
            )
        }
