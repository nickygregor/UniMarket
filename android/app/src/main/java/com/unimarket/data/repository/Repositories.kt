...
class AuthRepository(
    ...
    suspend fun bearerToken(): String? =
        tokenManager.token.first()?.let { "Bearer $it" }
}
...
class ListingRepository(
    ...
    suspend fun getById(id: Int): Result<Listing> =
        safeCall { api.getListingById(id) }
...
class OrderRepository(
    ...
    suspend fun checkout(req: CheckoutRequest): Result<Order> {
        val t = token() ?: return Result.Error("Not authenticated")
        return safeCall { api.checkout(t, req) }
    }
